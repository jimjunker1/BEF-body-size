...
# dw_dat_summ %>% 
#   ggplot()+
#   geom_point(aes(x = lmbar_n, y = lb_m2, color = site_id))+
...
b_m_lm_pred = predict(b_m_lm, newdata = dw_dat_summ %>% select(site_id, lmbar_n))
...
b_breaks = seq.int(round(min(dw_dat_summ$lb_m2, na.rm = TRUE)), round(max(dw_dat_summ$lb_m2, na.rm = TRUE)), by = 2)
b_labels = format(exp(b_breaks), scientific = TRUE, digits = 2)
M_breaks = seq.int(round(min(dw_dat_summ$lmbar_n, na.rm = TRUE)), round(max(dw_dat_summ$lmbar_n, na.rm = TRUE)), by = 2)
...
b_m_pred_plot = b_m_pred %>% 
  ggplot()+
  geom_path(aes(x = lmbar_n, y = .epred, group = .draw), color = 'darkgrey', alpha = 0.5)+
  geom_point(data = dw_dat_summ, aes(x = lmbar_n, y = lb_m2, fill = site_id), shape = 21, color = 'black', alpha = 0.5, position = position_jitter(width = 0.02))+
  geom_point(data = dw_dat_summ %>% summarise(lmbar_n = mean(lmbar_n),
                                              lb_m2 = mean(lb_m2, na.rm = TRUE),.by = 'site_id'),
             aes(x = lmbar_n, y = lb_m2, fill = site_id), color = 'black', shape = 21, size = 4)+
  scale_x_continuous(name = expression(paste(over("M"),["N-weighed"])),
                     breaks = M_breaks,
                     labels = M_labels)+#,
                     # limits = c(3.91,5))+
  scale_y_continuous(name = expression("Biomass (mg"~m^-2*")"),

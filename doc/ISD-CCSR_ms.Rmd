...
dw_dat = readRDS(here('data/dat_2022_clauset.rds')) %>% 
  ...
  select(dw, no_m2, site_id, collect_date, sample_id, log_gpp = "log_gpp_s", log_om = "log_om_s", mat_s) %>% 
...
dw_dat_temp = dw_dat %>% 
  ...
  summarise(n_tot = sum(n_sum),
            ...
            b_m2 = sum(b_sum),
            lb_m2 = log(b_m2),
            log_gpp = unique(log_gpp),
            log_om = unique(log_om),
            mat_s = unique(mat_s),
...
b_s_lm_pred = predict(b_s_lm, newdata = dw_dat_summ %>% select(site_id, hill_0))
...
b_breaks = seq.int(round(min(dw_dat_summ$lb_m2, na.rm = TRUE)), round(max(dw_dat_summ$lb_m2, na.rm = TRUE)), by = 2)
b_labels = format(exp(b_breaks), scientific = TRUE, digits = 2)
# S_breaks = seq.int(floor(min(log(dw_dat_summ$hill_0))), ceiling(max(log(dw_dat_summ$hill_0))), by = 0.25)
...
b_s_pred_plot = b_s_pred %>% 
  ...
  geom_point(data = dw_dat_summ, aes(x = log(hill_0), y = lb_m2, fill = site_id), shape = 21, color = 'grey', alpha = 0.5, position = position_jitter(width = 0.02))+
  geom_point(data = dw_dat_summ %>% summarise(hill_0 = mean(hill_0),
...
# b_m_lm_pred = predict(b_m_lm, newdata = dw_dat_summ %>% select(site_id, lmbar_b_met))

b_m_pred = dw_dat_summ %>% 
  modelr::data_grid(site_id = site_id,
...
b_breaks = seq.int(round(min(dw_dat_summ$lb_m2, na.rm = TRUE)), round(max(dw_dat_summ$lb_m2, na.rm = TRUE)), by = 2)
b_labels = format(exp(b_breaks), scientific = TRUE, digits = 2)
# M_breaks = seq.int(round(min(dw_dat_summ$lmbar_b_met, na.rm = TRUE)), round(max(dw_dat_summ$lmbar_b_met, na.rm = TRUE)), by = 2)
...
b_m_pred_plot = b_m_pred %>% 
  ...
  geom_point(data = dw_dat_summ, aes(x = lmbar_b_met, y = lb_m2, fill = site_id), shape = 21, color = 'grey', alpha = 0.5, position = position_jitter(width = 0.02))+
  geom_point(data = dw_dat_summ %>% summarise(lmbar_b_met = mean(lmbar_b_met),
                                              lb_m2 = mean(lb_m2, na.rm = TRUE),.by = 'site_id'),
...
NM_met_bwt_pred = dw_dat_summ %>% 
  ...
  left_join(dw_dat_summ %>% select(site_id, lmbar_b_met_s, ln_tot_s, lambda_med), by = 'lmbar_b_met_s') %>% 
  mutate(lmbar_b_met = lmbar_b_met_s * sd(dw_dat_summ$lmbar_b_met, na.rm = TRUE) + mean(dw_dat_summ$lmbar_b_met, na.rm = TRUE),
         ln_tot = ln_tot_s * sd(dw_dat_summ$ln_tot, na.rm = TRUE) + mean(dw_dat_summ$ln_tot, na.rm = TRUE),
         pred = pred_s * sd(dw_dat_summ$ln_tot, na.rm = TRUE) + mean(dw_dat_summ$ln_tot,na.rm = TRUE)) %>% 
...
N_breaks = seq.int(round(min(dw_dat_summ$ln_tot, na.rm = TRUE)), 20, by= 3)#round(max(dw_dat_summ$ln_tot, na.rm = TRUE)20), by = 3)
...
dw_dat_summ %>% 
  ...
  geom_point(data = dw_dat_summ %>% summarise(lmbar_b_met = mean(lmbar_b_met, na.rm = TRUE), 
...
b_m_marg = ggeffects::predict_response(b_s_full_lmm, terms = dw_dat_summ %>% select(lambda_med), margin = 'marginalmeans', type = 'fixed')
...
b_lambda_pred_plot = b_m_marg %>% 
  ...
  geom_point(data = dw_dat_summ, aes(x = lambda_med, y = lb_m2, fill = site_id), shape = 21, color = 'grey', alpha = 0.5)+
  geom_point(data = dw_dat_summ %>% summarise(
...
N_breaks = seq.int(round(min(dw_dat_summ$ln_tot, na.rm = TRUE)), 33, by= 3)#round(max(dw_dat_summ$ln_tot, na.rm = TRUE)20), by = 3)
N_labels = c(format(exp(N_breaks[N_breaks <= 9]), scientific = FALSE, digits = 1), format(exp(N_breaks[N_breaks >9]), scientific = TRUE, digits = 2))
...
NM_met_bwt_pred %>% 
  left_join(dw_dat_summ) %>% 
  ggplot()+
  geom_point(aes(x = log_gpp, y = resid_theory))

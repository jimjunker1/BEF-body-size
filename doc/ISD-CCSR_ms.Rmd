...
lambdas = read_csv(here("data/post_sample_lambdas.csv")) %>% 
  select(sample_int = sample_id, lambda_med = '.epred', lower_lambda = '.lower', upper_lambda = '.upper') 

dw_dat_summ = merge(dw_dat_summ, lambdas, by = 'sample_int', all = TRUE) %>% 
...
```{r richness models}
output = capture.output(brm(bf(lb_m2 ~ log(hill_0)),# + (1|site_id)),
                        data = dw_dat_summ,
...
b_s_pred_plot = b_s_pred %>% 
  ...
  geom_point(data = dw_dat_summ %>% summarise(hill_0 = unique(hill_0),

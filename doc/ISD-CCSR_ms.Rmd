...
x_axis_label =paste0("$\\overline{M$sup{0.75}$}$","$sub{B weighed}$","$\\quad$", "(mg ","$ind^{-1})$")
...
dw_dat_summ %>% 
  ...
  scale_x_continuous(name = TeX(x_axis_label, output = 'character'),
                     breaks = M_breaks,
                     labels = M_labels,
                     limits = c(-0.38,7.6))+
...
lambda_text_annotation = paste0("\\lambda","$\\approx$",eval(round(b_s_lmm_fix,2)))
...
saveRDS(b_s_pred_plot, here('doc/plots/b_lambda_pred_plot.rds'))

...
text_annotation = paste("~Theta ==",eval(round(b_s_lm_fix,2)))
...
b_s_pred_plot = b_s_pred %>% 
  ...
  annotate('text', x = -Inf, y = Inf, vjust = 1, hjust = 0, parse = TRUE, label = text_annotation )+
...
dw_dat_summ %>% 
  ggplot()+
  geom_point(aes(x = lmbar_b_met, y = lb_m2))
...
M_labels = format(exp(M_breaks), scientific = TRUE, digits = 2)

text_annotation = paste("~Theta ==",eval(round(b_m_lm_fix,2)))
...
b_m_pred_plot = b_m_pred %>% 
  ...
  scale_x_continuous(name = expression(over(,"M"["B-weighed"]^0.75)),
                     breaks = M_breaks,
                     labels = M_labels)+#,
                     # limits = c(3.91,5))+
  scale_y_continuous(name = expression("Biomass (mg"~m^-2*")"),

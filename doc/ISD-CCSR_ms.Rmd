...
dw_dat = readRDS(here("data/dat_fish_inverts_dw_allyears.rds")) %>% 
  ...
  mutate(collect_date = as.Date(collect_date),
         ...
         b_sum = sum(b_m2),
         b_rel = b_m2/b_sum,
         .by = c('site_id', 'sample_int','collect_date'))
...
dw_dat_summ = dw_dat %>% 
  ...
  summarise(n_tot = sum(n_sum),
            ...
            .by = c('site_id','sample_int','year_month')) %>% 
...
lambdas = readRDS(here("data/lambda_table.rds")) %>% 
  select(sample_int = sample_id, lambda_med = lambda_median, lower_lambda = '.lower', upper_lambda = '.upper') 

dw_dat_summ = merge(dw_dat_summ, H_dat %>% select(-all_of(taxa_list)), by = c('site_id','sample_int'), all = TRUE)
...
# S_breaks = seq.int(round(min(log(dw_dat_summ$hill_0))), round(max(log(dw_dat_summ$hill_0))), by = 0.25)
S_breaks = log(c(50, 75, 100, 125, 150))

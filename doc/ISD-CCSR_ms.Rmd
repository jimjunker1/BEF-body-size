...
# dw_dat_summ %>%
#   ggplot()+
#   geom_point(aes(x = lmbar_b_met, y = lb_m2, color = site_id))+
...
dw_dat_summ %>% 
  ...
  scale_y_continuous(name = expression("Abundance (individuals    "~m^-2~")"),
                     ...
                     limits = c(0,20))+
  scale_fill_viridis(discrete = TRUE)+
  annotate('text', x= -Inf, y = Inf, vjust = 5, hjust = 0, parse = TRUE, label = NM_text_annotation, family = 'serif' )+
...
plot(conditional_effects(b_s_full_lmm, points = TRUE, method = "posterior_predict"))
bayes_R2(b_s_full_lmm)
plot_predictions(b_s_full_lmm, condition = c('lambda_med'))
